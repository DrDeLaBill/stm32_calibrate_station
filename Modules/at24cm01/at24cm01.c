...
{
#if EEPROM_DEBUG
    printTagLog(EEPROM_TAG, "eeprom read: begin (addr=%lu, length=%u)\n", addr, len);
#endif
...
#if EEPROM_DEBUG
    printTagLog(EEPROM_TAG, "eeprom read: device i2c address - 0x%02x\n", dev_addr);
#endif

    HAL_StatusTypeDef status = HAL_BUSY;
    util_old_timer_t timer = { 0 };
    util_old_timer_start(&timer, GENERAL_TIMEOUT_MS);
    while (util_old_timer_wait(&timer)) {
        status = HAL_I2C_IsDeviceReady(&EEPROM_I2C, dev_addr, 1, GENERAL_TIMEOUT_MS);
    ...
    if (status != HAL_OK) {
#if EEPROM_DEBUG
        printTagLog(EEPROM_TAG, "eeprom wait ready: i2c error=0x%02x\n", status);
#endif
        return EEPROM_ERROR_BUSY;
    }

    status = HAL_I2C_Mem_Read(&EEPROM_I2C, dev_addr, (uint16_t)(addr & 0xFFFF), I2C_MEMADD_SIZE_16BIT, buf, (uint16_t)len, GENERAL_TIMEOUT_MS);
    if (status != HAL_OK) {
#if EEPROM_DEBUG
        printTagLog(EEPROM_TAG, "eeprom read: i2c error=0x%02x\n", status);
#endif
        return EEPROM_ERROR;
    }
...
{
#if EEPROM_DEBUG
    printTagLog(EEPROM_TAG, "eeprom write: begin (addr=%lu, length=%u)\n", addr, len);
#endif
...
#if EEPROM_DEBUG
    printTagLog(EEPROM_TAG, "eeprom write: device i2c address - 0x%02x\n", dev_addr);
#endif
    ...
    status = HAL_I2C_Mem_Write(&EEPROM_I2C, dev_addr, (uint16_t)(addr & 0xFFFF), I2C_MEMADD_SIZE_16BIT, buf, (uint16_t)len, GENERAL_TIMEOUT_MS);
    if (status != HAL_OK) {
#if EEPROM_DEBUG
        printTagLog(EEPROM_TAG, "eeprom write: i2c error=0x%02x\n", status);
#endif
        return EEPROM_ERROR;
    }
